...
export default function InfiniteArchiveGrid({ projects = [] }: { projects: Project[] }) {
  ...
  // -- Mobile Vertical Infinite Scroll Refs --
  ...
  // Base duplications for Infinite Scroll
  ...
  // GSAP Virtual Infinite Scroll (Desktop Horizontal & Mobile Vertical)
  useGSAP(() => {
    ...
    if (isDesktop && containerRef.current && trackRef.current) {
      ...
      const ticker = () => {
        ...
        if (isIdle) autoScrollOffset -= 1.5;
        ...
        while (currentScroll <= -minWidthPx) {
          ...
          logicalCards.push(logicalCards.shift()!);
        }
        while (currentScroll > 0) {
          currentScroll -= minWidthPx;
          autoScrollOffset -= minWidthPx;
          logicalCards.unshift(logicalCards.pop()!);
        }
        ...
        for (let i = 0; i < logicalCards.length; i++) {
          ...
          gsap.set(card, {
            x: (card as any)._x,
            width: `${(card as any)._widthPx}px`,
            height: `${(card as any)._heightVh}vh`,
  ...
  return (
    <div key={activeYear} className="min-h-[100svh]">
      
      {/* Hide Global Noise Overlay on Desktop only */}
      <style>{`
      ...
      {/* ── Fixed Header ────────────────────────────────────────────── */}
      <div 
        className="fixed top-0 left-0 right-0 z-50 flex justify-between items-start pointer-events-none mix-blend-difference text-white"
        style={{ 
          paddingLeft: PAD_LEFT, paddingRight: PAD_LEFT, paddingTop: "clamp(1.25rem, 3vh, 2.5rem)"
        }}
      >
        <button onClick={() => router.push("/#work")} style={{ background: "none", border: "none", color: "white", cursor: "pointer", fontFamily: "inherit", fontSize: "0.75rem", letterSpacing: "0.3em", textTransform: "uppercase", display: "flex", alignItems: "center", gap: "0.5rem", pointerEvents: "auto" }}>
      ...
      {/* ── Fixed Left Typography ─────────────────────────────────────── */}
      <div 
        className="fixed z-40 pointer-events-none mix-blend-difference text-white flex flex-col"
        style={{ 
          left: 0, 
          top: "25vh", // Kembali ke 25% dari atas (75% di atas layar) sesuai request
      ...
      {/* ── DESKTOP LAYOUT (Infinite Virtual Scroll - Light Mode) ─────── */}
      <div className="hidden lg:block bg-[#f5f5f5] min-h-screen text-black font-montserrat">
        <section ref={containerRef} className="fixed top-0 left-0 w-full h-screen overflow-hidden bg-[#f5f5f5]">
          <div className="absolute top-0 left-0 w-full h-full flex items-end pb-[10vh] pointer-events-none">
            <div ref={trackRef} className="relative w-full h-full pointer-events-auto border-b border-black/10">
